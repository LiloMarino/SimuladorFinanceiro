...
from sqlalchemy import BigInteger, Date, DateTime, Double, Enum, ForeignKeyConstraint, Identity, Integer, Numeric, PrimaryKeyConstraint, Text, UniqueConstraint, Uuid
...
class FixedIncomeAsset(Base):
    ...
    investment_type: Mapped[str] = mapped_column(Enum('CDB', 'LCI', 'LCA', 'TESOURO_DIRETO', name='investment_type'))
    rate_type: Mapped[str] = mapped_column(Enum('SELIC', 'IPCA', 'CDI', 'PREFIXADO', name='rate_type'))
    maturity_date: Mapped[datetime.date] = mapped_column(Date)
    interest_rate: Mapped[Optional[decimal.Decimal]] = mapped_column(Numeric(20, 6))

    event_fixed_income: Mapped[List['EventFixedIncome']] = relationship('EventFixedIncome', back_populates='asset')


class IpcaHistory(Base):
    __tablename__ = 'ipca_history'
    __table_args__ = (
        PrimaryKeyConstraint('ref_month', name='ipca_history_pkey'),
    )
...
class SelicHistory(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint('rate_date', name='selic_history_pkey'),
    )
...
class Stock(Base):
    ...
    event_equity: Mapped[List['EventEquity']] = relationship('EventEquity', back_populates='stock')
    stock_price_history: Mapped[List['StockPriceHistory']] = relationship('StockPriceHistory', back_populates='stock')
...
class Users(Base):
    ...
    event_cashflow: Mapped[List['EventCashflow']] = relationship('EventCashflow', back_populates='user')
    event_equity: Mapped[List['EventEquity']] = relationship('EventEquity', back_populates='user')
    event_fixed_income: Mapped[List['EventFixedIncome']] = relationship('EventFixedIncome', back_populates='user')
...
class EventCashflow(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE', onupdate='CASCADE', name='event_cashflow_user_id_fkey'),
        PrimaryKeyConstraint('id', name='event_cashflow_pkey')
    )

    id: Mapped[int] = mapped_column(BigInteger, Identity(always=True, start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), primary_key=True)
    user_id: Mapped[int] = mapped_column(Integer)
    event_type: Mapped[str] = mapped_column(Enum('DEPOSIT', 'WITHDRAW', 'DIVIDEND', name='cashflow_event_type'))
...
class EventEquity(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['stock_id'], ['stock.id'], ondelete='RESTRICT', onupdate='CASCADE', name='event_equity_stock_id_fkey'),
        ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE', onupdate='CASCADE', name='event_equity_user_id_fkey'),
    ...
    event_type: Mapped[str] = mapped_column(Enum('BUY', 'SELL', name='equity_event_type'))
...
class EventFixedIncome(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['asset_id'], ['fixed_income_asset.id'], ondelete='RESTRICT', onupdate='CASCADE', name='event_fixed_income_asset_id_fkey'),
        ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE', onupdate='CASCADE', name='event_fixed_income_user_id_fkey'),
    ...
    event_type: Mapped[str] = mapped_column(Enum('BUY', 'REDEEM', name='fixed_income_event_type'))
    amount: Mapped[decimal.Decimal] = mapped_column(Numeric(20, 6))
    event_date: Mapped[datetime.date] = mapped_column(Date)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(True))

    asset: Mapped['FixedIncomeAsset'] = relationship('FixedIncomeAsset', back_populates='event_fixed_income')
...
class Snapshots(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE', onupdate='CASCADE', name='snapshots_user_id_fkey'),
...
class StockPriceHistory(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['stock_id'], ['stock.id'], ondelete='RESTRICT', onupdate='CASCADE', name='stock_price_history_stock_id_fkey'),
